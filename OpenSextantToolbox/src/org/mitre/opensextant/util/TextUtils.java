...
public class TextUtils {
	 ...
	 * @return
	 */
	public static int[] get_text_window(int offset, int matchlen, int textsize,
			int width) {
		/*
	 ...
	 * @return
	 */
	public static String fast_replace(String buf, String replace,
			String substitution) {
	...
	public static String normalize_text_entity(String str) {
		...
		int end = s2;
		while (s1 < s2
				&& !(Character.isLetter(chars[s1]) || Character
						.isDigit(chars[s1]))) {
			++s1;
		}
		// No text found
		...
		while (s2 > s1
				&& !(Character.isLetter(chars[s2]) || Character
						.isDigit(chars[s2]))) {
			--s2;
		}
		if (s1 == 0 && s2 == end) {
