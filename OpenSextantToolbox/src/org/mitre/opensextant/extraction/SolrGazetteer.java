...
public class SolrGazetteer {
	...
	private void loadFeatureMetaMap() throws IOException {
		java.io.InputStream io = SolrGazetteer.class
				.getResourceAsStream("/feature-metadata-2013.csv");
		java.io.Reader featIO = new InputStreamReader(io);
		CsvMapReader featreader = new CsvMapReader(featIO,
	...
	private void loadCountryNameMap() throws IOException {
		java.io.InputStream io = SolrGazetteer.class
				.getResourceAsStream("/country-names-2013.csv");
		java.io.Reader countryIO = new InputStreamReader(io);
		CsvMapReader countryMap = new CsvMapReader(countryIO,
				CsvPreference.EXCEL_PREFERENCE);
	...
	public static final Place UNK_Country = new Place("UNK", "invalid");
	/**
	...
	protected void loadCountries() throws SolrServerException {
		...
		for (SolrDocument gazEntry : docList) {
			...
			C.setLatitude(xy[0]);
			C.setLongitude(xy[1]);
			C.addAlias(C.getPlaceName()); // don't loose this entry as a likely
											// variant.
	 ...
	 * @throws MatcherException
	 */
	public List<Place> search(String place, boolean as_solr)
	...
	public static void main(String[] args) throws Exception {
		...
		String SOLR_HOME = OPENSEXTANT_HOME + File.separator + ".."
		...
		try {
			...
			for (Place c : countries.values()) {
				System.out.println(c.getPlaceID() + " = " + c.getPlaceName()
			...
			for (Place pc : matches) {
				System.out.println(pc.toString()
						...
						+ gaz.getFeatureName(pc.getFeatureClass(),
								pc.getFeatureCode()));
			}
		} catch (Exception err) {
			err.printStackTrace();
