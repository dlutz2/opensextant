...
public class TaxonMatcher {
    ...
    public List<TaxonMatch> tagText(String buffer, String docid) throws MatcherException {
        ...
        try {
            ...
            throw new MatcherException("Failed to tag document", err);
        }
        ...
        for (NamedList<?> tag : tags) {
            ...
            m.setText(buffer.substring(x1, x2));
    ...
    public void testDoc(String buf) throws MatcherException {
        List<TaxonMatch> matches = this.tagText(buf, "test");

        for (TaxonMatch tx : matches) {
