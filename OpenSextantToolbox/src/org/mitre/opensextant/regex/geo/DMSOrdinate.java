...
public final class DMSOrdinate {
	 ...
	 * @param text
	 */
	public DMSOrdinate(java.util.Map<String, String> elements, boolean islat,
			String text) {
		...
		/*
		 *
		 * } catch (Exception fmterr) { // do nothing // possible report parse
		 * errors in debug mode. throw new XCoordException("Format Error",
		 * fmterr); }
		 *
		 * if (!_parse_state) { throw new
		 * XCoordException("Unable to parse match due to " + (islat ? "LAT" :
		 * "LON")); }
		 */
		set_normalized_text();
		value = toDecimal();
     ...
     *
     */
	protected static final DecimalFormat fieldDec = new DecimalFormat(
			"00.######");
	/**
	...
	protected void normalize_hemisphere(String text,
		...
		int hemi = (islat ? getLatHemisphereSign(elements)
				: getLonHemisphereSign(elements));
	 ...
	 * @param hemi_sign
	 */
	public DMSOrdinate(int deg, int min, int sec, int msec, boolean islat,
			int hemi_sign) {
	 ...
	 */
	public DMSOrdinate(String deg, String min, String sec, boolean islat,
			int hemi_sign) throws java.text.ParseException {
		if (deg == null) {
	...
	public double toDecimal() {
		...
		return (double) hemisphere.polarity
