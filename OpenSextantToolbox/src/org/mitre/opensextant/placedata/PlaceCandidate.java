...
public class PlaceCandidate implements Serializable {
	...
	/**
	 * TextEntity.getText() the actual text is the place name attr Not to be
	 * confused with the place names of candidate places enumerated under
	 * getPlaces()
	 */
	...
	public boolean possibleCountry(String cc) {
		for (Place p : rankedPlaces) {
			if (p.getCountryCode() != null
					&& p.getCountryCode().equalsIgnoreCase(cc)) {
				return true;
			}
		}
		return false;
