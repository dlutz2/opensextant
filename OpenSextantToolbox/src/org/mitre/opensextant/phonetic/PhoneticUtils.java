...
public class PhoneticUtils {
	...
	static final Pattern CLEAN_WORD_RIGHT = Pattern
			.compile("[^\\p{L}\\p{Nd}]+$");
	static final Pattern CLEAN_WORD_LEFT = Pattern
			.compile("^[^\\p{L}\\p{Nd}]+");
	// TODO: convert bytes here to \\u patterns
	static final Pattern CLEAN_WORD_PUNCT = Pattern
			.compile("[\"'.`\\u00B4\\u2018\\u2019]");
	/**
	...
	public static String script(char c) {
		...
		if (blockName.contains("BLOCK") || blockName.contains("BOX")
	...
	public static void main(String[] args) throws Exception {
		String[] testWords = { "Sa‘īd", "'Wombat", "Wombat'", "***[Image Bar",
		  "99 luft Balloons...", "St. John U.S.S.R.", "Jose-Enrique",
		  "Al 'farqu Ha'na", "Bélow", "Colón", "Ås", "Bäck", "Ön", "Tōp",
		  "Nõmme", "Çat", "Çavuş", "Góry", "Onça", "Röd", "Armonía",
		  "Poço", "Būm", "Våge" };
		System.out.println("WORD\tNO PUNCT\tNO ACCENTS\tNO CASE");
		for (String wd : testWords) {
			System.out.println(wd + " ->" + removePunctuation(wd) + "\t"
					+ removeDiacritics(wd) + "\t" + removeCase(wd));
		}
	}
}
