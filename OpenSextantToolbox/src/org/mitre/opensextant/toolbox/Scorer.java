...
public class Scorer {
	...
	double totalWeight = nameWeight + geocoordWeight + featureTypeWeight
			+ adminStructureWeight;
	...
	// everything by zero
	Double MinEvidenceValue = .00001;
	// a flag value to indicate no evidence was found
	String NO_EVIDENCE = "NO_EVIDENCE";
	// name similarity scores
	double NameSimilarityCaseInsens = 0.90;
	double NameSimilarityFloor = 0.50;
	...
	private double scoreFeatureType(PlaceEvidence evidence, Place place) {
		...
		if (evidenceFClass.equals(gazetteerFClass)
				&& evidenceFCode.equals(gazetteerFCode)) {
			return 1.0 * weight;
		}
