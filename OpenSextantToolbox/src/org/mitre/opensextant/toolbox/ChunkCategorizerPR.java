...
public class ChunkCategorizerPR extends AbstractLanguageAnalyser implements
	...
	private void categorize(Annotation np, AnnotationSet vocab,
		...
		// npEndNode.getOffset()));
		List<Annotation> entitySetHead = gate.Utils.inDocumentOrder(entities
				.get(headStartNode.getOffset(), headEndNode.getOffset()));
		...
		if (entitySetHead.size() > 0) {
			npFeatures.put("EntityType", entitySetHead.get(0).getFeatures()
					.get("hierarchy"));
