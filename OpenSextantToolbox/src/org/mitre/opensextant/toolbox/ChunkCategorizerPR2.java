...
public class ChunkCategorizerPR2 extends AbstractLanguageAnalyser implements
	...
	public void execute() throws ExecutionException {
		...
		AnnotationSet entitySet = document.getAnnotations().get(null,
				entityFeatureNameSet);
	...
	private void categorizeTokens(AnnotationSet tokenSet,
		...
		for (Annotation a : tokenSet) {
			...
			if (vSet.size() >= 1) {
				...
				String tmpCatHier = (String) tmpVocab.getFeatures().get(
						"hierarchy");
	...
	private void categorize(Annotation np) {
		List<String> categories = (List<String>) np.getFeatures().get(
				"CategorySequence");
		String reducedCatSeq = (String) np.getFeatures().get(
				"CategorySequence_Reduced");
		String cat = "";
		String type = "";
		String hier = "";
	...
	private void addToCorefMap(Annotation np) {
		...
		List<String> propers = (List<String>) np.getFeatures().get(
				"ProperSequence");
