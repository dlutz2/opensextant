...
public class CantileverPR extends AbstractLanguageAnalyser implements
	...
	public void execute() throws ExecutionException {
		...
		for (Annotation candAnno : candidateSet) {
			...
			PlaceCandidate pc = (PlaceCandidate) candAnno.getFeatures().get(
	...
	private List<PlaceEvidence> collectDocumentEvidence(AnnotationSet annotSet) {
		...
		for (Annotation countryAnno : countrySet) {
			String tmpCC = (String) countryAnno.getFeatures()
					.get("countryCode");
			String countryName = (String) countryAnno.getFeatures().get(
					"string");
		...
		for (Annotation capitalAnno : capitalSet) {
			String tmpCC = (String) capitalAnno.getFeatures()
					.get("countryCode");
			String capitalName = (String) capitalAnno.getFeatures().get(
					"string");
			// TODO replace with abbreviation when added to gazetteer
		...
		for (Annotation adminAnno : adminSet) {
			...
			String tmpAdmCode = (String) adminAnno.getFeatures()
					.get("adm1code");
			...
			if (adminName != null && adminName.length() > 3
					&& tmpAdmCode != null && tmpAdmCode.length() > 1) {
				if (!countryCounts.keySet().contains(tmpCC)) {
					countryCounts.put(tmpCC, 0);
				}
