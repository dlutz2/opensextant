	...
	public static void start() {
		...
		if (isConfigured) {
			...
			if (isRemote) {
				...
				try {
					File solr_xml = new File(homeLocation + File.separator
							+ "solr.xml");
					CoreContainer solrContainer = new CoreContainer(
							homeLocation);
					solrContainer.load(homeLocation, solr_xml);
					;
					EmbeddedSolrServer server = new NoSerializeEmbeddedSolrServer(
							solrContainer, "");
					solrServer = server;
					isStarted = true;
