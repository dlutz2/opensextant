...
/**
 * DMSOrdinate represents all the various fields a WGS84 cartesian coordinate could have. degree/minute/second, as well
 * as fractional minutes and fractional seconds. * Patterns may have symbols which further indicate if a pattern is a
...
public final class DMSOrdinate {
  ...
  // Log object
  private static Logger log = LoggerFactory.getLogger(DMSOrdinate.class);
  ...
  /**
   * DMS ordinates can be made up of degrees, minutes, seconds and then decimal minutes and decimal seconds
   *
   * fractional minutes dddd are formatted as ".dddd" then parsed; ADD to existing minutes fractional seconds dddd are
   * formatted as ".dddd" then parsed; ADD to existing seconds
   *
   * decimal minutes d.ddd.. are parsed as float, divide by 60 to get # of degrees decimal seconds d.ddd.. "" "" ,
   * divide by 3600 to get # of degrees
   *
   *
  ...
  public DMSOrdinate(java.util.Map<String, String> elements, boolean islat, String text) {
    ...
    /*
     * degLat, degLon dd, ddd variable length minLat, minLon 0-59, "" secLat, secLon 0-59, "" decMinLat, decMinLon 0.00
     * to 59.99... variable length, arbitrary precision decMinLat3, decMinLon3 0-000 to 59-999... variable length,
     * arbitrary precision decSecLat, decSecLon --- does not exist; need test cases for sub-second accuracy most use
     * decimal degree d.dddddd...
     *
     * dmsDegLat, dmsDegLon "dd", "ddd" dmsMinLat, dmsMinLon, "00" to "59" dmsSecLat, dmsSecLon, "00" to "59"
     * fractMinLat, fracMinLon d to dddddd part of some other string, but represents 0.ddddd minutes fractMinLat3,
     * fractMinLon3 ddd to dddddd "" ""
     */
    _elements = elements;
    ...
    /*
     *
     * } catch (Exception fmterr) { // do nothing // possible report parse errors in debug mode. throw new
     * XCoordException("Format Error", fmterr); }
     *
    if (!_parse_state) { throw new XCoordException("Unable to parse match due to " + (islat ? "LAT" : "LON")); }
     */
    set_normalized_text();
    value = toDecimal();
  }
  ...
  /**
   * Get back a normalized version of what you found. Yield zero-padded version of coordinate, e.g., if DMS xdeg M' SS"
   * is parsed we want xxx:MM:SS fully padded
   *
   * 45 33N-00811 given 45:33.00811 normalized, DEG:MIN.mmmmm
   *
   * Full format should be:
   *
   * hDDD.ddd hDDD:MM.mmm hDDD:MM.mmm:SS.sss unlikely -- if this happens -- both fractional minutes and fractional
   * seconds, pattern has errors hDDD:MM:SS.sss
   *
  ...
  protected void set_normalized_text() {
    ...
    /*
     * if (this.is_latitude) { buf.append(field.format(this.degrees)); // LAT degree } else {
  ...
  /**
   * As a result of this, the fsec field text will be saved with decimal point. Convert "-888" to the decimal part of a
   * value, e.g., "0.888"
   */
  ...
  public final boolean digest_latitude_match() {
    ...
    Float min_fract2 = getFractionValue("fractMinLat3", "fmin"); // variation
    // 2, is
    // a
    // 3-digit
    // or
  ...
  public final boolean digest_longitude_match() {
    ...
    Float min_fract2 = getFractionValue("fractMinLon3", "fmin"); // variation
    // 2, is
    // a
    // 3-digit
    // or
  ...
  protected void normalize_hemisphere(String text, java.util.Map<String, String> elements, boolean islat) {
    ...
    if (!has_hemi) {
      ...
      /*
       * if (hasSymbols()) { hemi = POS_HEMI; // Un-specified hemisphere defaults to POSITIVE } else { hemi = POS_HEMI;
       * }
       */
    }
  ...
  /**
     *
     */
  public static final String[] COORDINATE_SYMBOLS = {"��", "��", "'", "\"", ":", "lat", "lon", "geo", "coord", "deg"};
   ...
   *
   * hemiLon W, E group used mostly for DMS, DM, DD formats hemiLonSign +, - group allowed only for specific formats;
   * +/- may appear before any number not just coords. hemiLonPre W, E, +, -
   *
   * @param elements
   *          list of trimmed components of the match
   * @return
   ...
   */
  public DMSOrdinate(String deg, String min, String sec, boolean islat, int hemi_sign) throws java.text.ParseException {
    if (deg == null) {
  ...
  /**
   * toDD() Return the decimal value to the extent it makes sense. That is, calculating minutes or seconds where MIN or
   * SEC fields are out of bounds for those units does not make sense, return at least Deg or Deg/Min.
   *
  ...
  public double toDecimal() {
    // float value = 0.0f;
    // boolean too_high = (is_latitude && degrees >= LAT_MAX) |
    // (!is_latitude && degrees >= LON_MAX);
    // if (too_high) {
    // return value;
    // }
    if (minutes >= 60) {
