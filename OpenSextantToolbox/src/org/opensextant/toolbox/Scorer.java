...
/**
 * The Scorer attempts to quantify (using a range between -1.0 and 1.0) the similarity between a Place Candidate which
 * has been found in a document with matches from the gazetteer. The score consists of a weighted average of four one
 * dimensional similarity measurements:
 * <ul>
 * <li>The similarity between the name in the document and the name in the gazetteer</li>
 * <li>The geometric distance between a geocoord associated with the name in the document and the geocoordinate of the
 * name in the gazetteer</li>
 * <li>The similarity/compatibility of feature type information associated with the name in the document and the type of
 * the feature in the gazetteer.</li>
 * <li>The similarity/compatibility between the admin structure associated with the name in the document and and the
 * structure of the feature in the gazetteer.</li>
 * </ul>
 */
public class Scorer {
  ...
  /**
   * The scoring function which calculates the similarity between a place and a single piece of evidence
   *
   * @param ev
   * @param place
   * @return
  ...
  /* ---------------------Name Similarity ------------------ */
  /**
   * Compare the name as found in the document with the name in the candidate as found in the gazetteer Exact match =1.0
   * Exact match (ignoring case) = <NAME_SIMILARITY_CASEINSENS> .... Any condition not caught in the above =
  ...
  /* ---------------------Geocoord Similarity ------------------ */
  /**
   * Compare the geocoord evidence from the document with the location (geocoord) of the candidate as found in the
   * gazetteer. Comparison is based on dist between evidence and gazetteer place. Range = -1.0 -> 1.0
   *
   * NOTE: these thresholds really should be modulated by FeatureType rather than be absolutes
   *
   * @param evidence
   * @param place
   * @return
  ...
  /**
   * Compare the similarity/compatibility of the feature type evidence as found in the document with the feature type
   * info from the place in the gazetteer. Range = -1.0 <-> 1.0
   *
   * @param evidence
   * @param place
   * @return
  ...
  /**
   * Compare the similarity/consistency of the administrative structure evidence found in the document with that of the
   * place from the gazetteer. Range = -1.0 -> 1.0
   *
   * @param evidence
   * @param place
   * @return
